...
### 15. Session publish gate — `backend/scripts/publish_session/`
Gates *when* a session becomes the dashboard's "latest" view. `cc-publish-session` (cron `*/30 20-23,0-9`) stamps `pl_session_release` for a session once its data is complete (indicator + press + meteo) **and** its NotebookLM audio is present in Drive → the dashboard flips to the new session **atomically, the same evening** (not the next-morning `display_date`). Morning fallback (past `display_date(T)` 09:00 UTC) releases data-only so it never freezes on yesterday.
- **Entrypoints**: `poetry run publish-session [--session-date|--window-days|--dry-run|--force]` (cron `*/30 20-23,0-9`).
- **Reads**: `pl_indicator_daily`, `pl_fundamental_article`, `pl_weather_observation`, Google Drive (audio presence via `audio_service`).
- **Writes**: `pl_session_release` (`session_date` PK, `published_at`, `has_audio`, `source`).
- **Consumed by**: `dashboard.py` `non-trading-days` endpoint — `latest_trading_day = MAX(display_date)` join `pl_session_release`, with **safe fallback** to `MAX(display_date) <= today` while the table is empty (non-breaking, ships dark).
- **Docs**: `runbooks/session-publish-gate.md`.
