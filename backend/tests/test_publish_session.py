...
# _decide never touches `session` once the completeness/audio helpers are
# patched — a typed None keeps the real signature strict without a live DB.
...
def test_complete_plus_audio_releases(monkeypatch):
    ...
    d = _decide(_NO_SESSION, T, NOW, force=False)
...
def test_complete_no_audio_before_deadline_skips(monkeypatch):
    ...
    d = _decide(_NO_SESSION, T, NOW, force=False)  # NOW < DEADLINE
...
def test_no_audio_past_deadline_falls_back(monkeypatch):
    ...
    d = _decide(_NO_SESSION, T, past, force=False)
...
def test_partial_data_past_deadline_still_releases(monkeypatch):
    ...
    d = _decide(_NO_SESSION, T, DEADLINE, force=False)
    assert d.action == "release"
    assert "fallback" in d.reason
...
def test_force_releases_regardless(monkeypatch):
    ...
    d = _decide(_NO_SESSION, T, NOW, force=True)
    assert d.action == "release"
...
def test_no_indicator_row_skips(monkeypatch):
    ...
    d = _decide(_NO_SESSION, T, DEADLINE, force=True)  # even --force can't publish
    assert d.action == "skip"
