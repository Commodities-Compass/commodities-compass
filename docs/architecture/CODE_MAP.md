...
- **Key invariants**: cross-contract fallback on rolls via `resolve_contract_for_date()` + `v_contract_data_chained` (`DISTINCT ON date ORDER BY oi DESC`); position defaults to MONITOR on null (benign) but logs ERROR on unrecognized values (never silent corruption); YTD + running-accuracy use `eval_horizon_for(algorithm)` — **regime is scored J+1**, everything else falls back to `YTD_EVAL_HORIZON_DAYS=4`; contract/algo lookups cached 5min, audio lookups 1h hit / 5min miss; judge diagnostics stay silent (not fabricated) on a date the overlay did not run, and `rationale` — the fuse trace — is deliberately **absent from the payload**, not merely unrendered.
