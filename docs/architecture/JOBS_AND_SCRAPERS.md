...
| **cc-intraday-monitor** | `*/5 8-16 * * 1-5` | shared | Barchart overview HTML (navigateur headless via AWS WAF, delayed ~10-12 min + cache CDN ≤300s) + `pl_derived_indicators` (S1/R1) + `ref_alert_rule` | `pl_contract_data_intraday` (append) + `aud_alert_event` + Telegram sendMessage | ✅ Actif (LIVE telegram) |
...
**Source** : `https://www.barchart.com/cmdty/data/fundamental/explore/IC345DRW.CS`. Navigateur headless (`_shared/barchart_browser.py`) + BeautifulSoup — httpx jusqu'au 2026-09-03, puis AWS WAF (202 + challenge JS) ; job 512Mi → 1Gi, parser inchangé. Validate native unit `60 Kg Bag` + multiplier 1 — fail-loud si drift.
...
**Description** : toutes les 5 min en séance de Londres, fetch le prix différé du front-month via un navigateur headless qui franchit l'AWS WAF (un seul chargement de la page overview, `raw.lastPrice` lu dans le JSON inline server-rendered, bloc sélectionné par **match exact du symbole**), append `pl_contract_data_intraday`, et compare aux niveaux « À surveiller » (S1/R1 de `pl_derived_indicators` à la **dernière session complétée** — les niveaux affichés sur le dashboard le jour même). Franchissement edge-triggered `(prev, curr)` → alerte **first-cross-only par (règle, session)**, dédup data-level `UNIQUE(rule_id, session_date, crossing_seq)` + `ON CONFLICT DO NOTHING` (un re-run ne re-spamme jamais). Delivery via interface `AlertSender` : `TelegramSender` (canal privé broadcast-only, un `sendMessage` = fan-out) ou `ConsoleSender` (`ALERT_CHANNEL=console`, shadow/dev).

⚠️ **httpx est mort le 2026-09-03.** Barchart a activé **AWS WAF** : chaque page répond `HTTP 202` avec un interstitiel de ~2 Ko qui charge `challenge.js` depuis `*.token.awswaf.com`, forge un cookie `aws-waf-token` en JS puis recharge. Aucun client httpx ne peut produire ce token. Le fetch passe désormais par un navigateur headless (`scripts/_shared/barchart_browser.py`, partagé avec `barchart-stocks-eu-scraper` : un seul contexte par run, le cookie de clearance est réutilisé), job 512Mi → 1Gi. **Le parser n'a pas bougé** — `parse_delayed_quote` est pur et le transport est injecté. `cc-barchart-scraper` était déjà sous Playwright et n'a jamais cassé.

⚠️ **Le two-step `core-api` est mort le 2026-09-01.** Barchart est passé derrière CloudFront (`cache-control: public, s-maxage=300`) ; un CDN ne relaie pas `Set-Cookie` sur une réponse mise en cache partagé, donc le cookie `XSRF-TOKEN` qu'exige `/proxies/core-api/v1/quotes/get` n'est plus obtenable sans navigateur — l'endpoint répond `403`. Toutes les pages HTML du site sont désormais sans cookie. Le job a échoué sur 100 % des ticks de la séance (22 events, 08:35→10:21 UTC) avant la bascule sur la lecture HTML, qui était le **plan B déjà documenté dans le spike d'origine**. Contrepartie : jusqu'à 300s de cache CDN s'ajoutent au différé Barchart (~10-12 min), soit ~17 min au pire ; `trade_time` porte l'âge réel et est loggé à chaque run.
