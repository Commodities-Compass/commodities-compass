...
- **Entrypoints**: `poetry run watchai-sync --source <watch-ai checkout> [--dry-run] [--target {local,prod}] [--skip-compute]`. Serving: `app/services/origin_balance.py` (pure bean-equivalent arithmetic, no I/O) + `origin_flow_service.py` (queries) → `api_v1/endpoints/origin.py` (`/dashboard/origin/{campaign,market-views}`).
