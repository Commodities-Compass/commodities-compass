...
- ⚠️ **httpx is dead since 2026-09-03.** Barchart turned on **AWS WAF**: every page answers `HTTP 202` with a ~2KB interstitial that loads `challenge.js` from `*.token.awswaf.com`, mints an `aws-waf-token` cookie in JS and reloads. No httpx client can produce that token. The fetch moved to a headless browser (`scripts/_shared/barchart_browser.py`, shared with `barchart-stocks-eu-scraper`; one context per run, clearance cookie reused), job 512Mi → 1Gi. The **parser is unchanged** — `parse_delayed_quote` is pure and the transport is injected, so the next posture change touches one function. `cc-barchart-scraper` was already on Playwright and never broke.
