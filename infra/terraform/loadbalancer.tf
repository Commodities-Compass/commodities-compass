...
#   staging.com-compass.com    → landing staging bucket (Cloud CDN)
#   com-compass.com + www      → landing prod bucket (Cloud CDN)
#
...
#
# Landing buckets + CDN configs live in landing.tf — kept separate to make
...
resource "google_compute_url_map" "main" {
  name    = "cc-url-map"
  project = var.project_id
  # Apex of the LB is the landing prod bucket — any unmatched host (e.g. someone
  # hitting the LB IP directly) lands on the marketing site, not on the app.
  default_service = google_compute_backend_bucket.landing_prod.id
  ...
  host_rule {
    hosts        = ["staging.com-compass.com"]
    path_matcher = "landing-staging"
  }

  host_rule {
    hosts        = ["com-compass.com", "www.com-compass.com"]
    path_matcher = "landing-prod"
  }
  ...
  path_matcher {
    ...
    default_service = google_compute_backend_bucket.landing_staging.id
  }
...
resource "google_compute_target_https_proxy" "main" {
  ...
  # Host header. Limits: max 100 certs per proxy, we use 4 (well below).
  ssl_certificates = [
    google_compute_managed_ssl_certificate.app.id,
    google_compute_managed_ssl_certificate.api.id,
    google_compute_managed_ssl_certificate.landing_staging.id,
