...
# R&D-only fallback resolution. Production never instantiates these directories
# (its data layer feeds ``pl_external_indicator`` directly via the
# ``EnsembleDataLoader`` Protocol). When this module is imported in prod with
# no R&D layout available, the path constants resolve to non-existent paths;
# all load_* functions return empty DataFrames in that case, so the module is
# safe to import everywhere.
def _resolve_repo_root() -> Path:
    env_root = os.environ.get("RND_COMPASS_REPO_ROOT")
    if env_root:
        return Path(env_root)
    # Walk up from this file looking for a sibling ``data/`` directory (the R&D
    # convention). Fall back to the package root if not found.
    here = Path(__file__).resolve()
    for parent in here.parents:
        if (parent / "data" / "external_data").exists():
            return parent
