...
# The Conviction row: routed regime + model probability + what the macro overlay
# did with the call. Until 2026-08-18 the row was backed by two ensemble keys
# (read:feature:{ensemble_diagnostics,specialist_votes}); they went with the
# endpoints. Grants for them may still sit in tenant_entitlement — the table is
# append-only and a stale grant is inert, since resolve_principal never validates
# a key against this catalogue.
...
# an explanation of how convinced the system is. It was backed by two ensemble
# keys, then briefly by three during the overlap, and now by one — and through
# all of it not a single tier template changed. That is what composing the bundle
# by commercial row buys: the implementation moves, the offer does not.
_CONVICTION = frozenset({FEATURE_JUDGE_OVERLAY})
