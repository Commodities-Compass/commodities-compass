-- regime algorithm v1.0.0 — algorithm_version + config seed.
--
-- NEW ARCHITECTURE => NEW pl_algorithm_version row (not an append on an existing
-- version). Per the handoff pitfall #1, the row ships INERT:
--     is_active = FALSE, compute_enabled = FALSE
-- so it can shadow-compute alongside the live ensemble without ever being served
-- to users. Cutover is a Compass-side decision (flip the flags) AFTER shadow-eval.
--
-- Idempotent: the NOT EXISTS guard skips both the version and its config rows.
-- Per-specialist models live in pl_model_artifact (BYTEA), looked up by name;
-- this seed carries only the router config + the specialist condition registry.
...
FROM pl_algorithm_version v,
     (VALUES
        -- Router thresholds (Layer 1) — all causal.
        ...
        -- Specialist condition registry (config-as-data; parallels the ensemble cluster rows).
