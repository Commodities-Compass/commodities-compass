...
- ⚠️ **httpx is dead since 2026-09-03.** Barchart turned on **AWS WAF**: every page answers `HTTP 202` with a ~2KB interstitial that loads `challenge.js` from `*.token.awswaf.com`, mints an `aws-waf-token` cookie in JS and reloads. No httpx client can produce that token. The fetch moved to a headless browser (`scripts/_shared/barchart_browser.py`, shared with `barchart-stocks-eu-scraper`; one context per run, clearance cookie reused), job 512Mi → 1Gi. **The User-Agent is mandatory**: headless Chromium's default UA says `HeadlessChrome/<v>` and AWS WAF answers it with a flat `403` — no challenge, nothing to solve. Proven same-machine/same-IP on 2026-09-03 (default UA → 403, realistic UA → 202). macOS hid it for a full afternoon because Playwright picks WebKit there (plain Safari UA), so the first cut passed locally and 403'd on all 32 Cloud Run ticks — while `cc-barchart-scraper`, which has forced this header for six months, kept working from the same IP. The shared module now pins `USER_AGENT` to `barchart_scraper.config.USER_AGENT` and a test fails if they drift or if it ever says "headless". **Readiness is positive, never an absence**: the first cut waited only for the challenge markers to vanish, and Cloud Run got a page that never carried them — "not challenged" read as "settled", an unusable page reached the parser in 288 ms, and the error blamed a layout drift. Each caller now declares the `ready_marker` that proves *its* page arrived (the contract symbol / `cmdty-quote-table`), and failures carry HTTP status + length + body snippet so the next move diagnoses itself. The **parser is unchanged** — `parse_delayed_quote` is pure and the transport is injected, so the next posture change touches one function. `cc-barchart-scraper` was already on Playwright and never broke.
...
- `/dashboard/*` - Trading dashboard data (all require auth):
  ...
  - `GET /dashboard/farmgate-price` - Official/guaranteed farmgate price (CIV/CCC + Ghana/COCOBOD). Publishes the **focus season** — `MAX(season_label)` in the table, i.e. the most recent season *either* origin has announced — and, per region, the price **in force** within it (latest effective ≤ date; the forthcoming price when the season hasn't started yet). Response is flat: `{date, season, civ: entry|null, ghana: entry|null}`; **`null` = that origin hasn't announced this season → the card says "En attente"**, it never falls back to last season's price. No sub-campaign split any more (there is one card per origin since 2026-09-05): a mid-crop row simply becomes the price in force from its effective date, and `campaign_type` rides along on the entry as a tag. ⚠️ Inserting a new season for one origin flips the other to pending until its own row lands. Append-only `pl_official_farmgate_price`; ops entry via `poetry run set-farmgate-price`. Mirrored sync-side by `scripts/_shared/farmgate_brief.py` for the brief — the two must stay in step. Distinct from the real terrain price (Programme Fondateur).
