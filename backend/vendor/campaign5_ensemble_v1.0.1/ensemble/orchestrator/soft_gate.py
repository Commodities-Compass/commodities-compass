...
class SoftGateConfig:
    ...
    # v1.0.1 structural guardrail: hard ceiling on the effective macro coupling.
    # Keeps (1 + alpha_macro*(-1)) > 0 so a contrarian specialist is down-weighted,
    # never zeroed (the 2026-05 unanimous-HEDGE collapse). Aligned with the prod
    # compass_softgate_alpha_macro_cap=0.9 — single source of truth, no double-cap.
    alpha_macro_cap: float = 0.9
...
class SoftGateOrchestrator:
    ...
    def decide(
        ...
        for name, pred in specialist_votes.items():
            ...
            # v1.0.1: clamp the effective macro coupling to alpha_macro_cap (< 1.0) so a
            # contrarian (macro_align=-1) is never fully zeroed before the vote.
            eff_alpha_macro = min(cfg.alpha_macro, cfg.alpha_macro_cap)
            weight = (
                base
                * (1.0 + eff_alpha_macro * macro_align)
