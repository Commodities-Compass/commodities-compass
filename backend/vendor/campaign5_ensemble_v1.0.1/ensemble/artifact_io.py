...
class DBArtifactLoader:
    ...
    _FIND_SQL = (
        "SELECT artifact_name FROM pl_model_artifact "
        "WHERE algorithm_version_id = :aid AND artifact_kind = :k "
        "AND artifact_name LIKE :pfx ORDER BY artifact_name LIMIT 1"
    )

    def find_name(self, artifact_kind: str, name_prefix: str) -> str | None:
        """First artifact_name of ``artifact_kind`` starting with ``name_prefix``, else None.

        Lets the pipeline resolve a cutoff-dated canonical_snapshot row (e.g.
        ``regime_tags_rd_<cutoff>``) from the DB without hardcoding the date.
        """
        row = self._session.execute(
            self._FIND_SQL,
            {"aid": self._aid, "k": artifact_kind, "pfx": f"{name_prefix}%"},
        ).first()
        return str(row.artifact_name) if row is not None else None
