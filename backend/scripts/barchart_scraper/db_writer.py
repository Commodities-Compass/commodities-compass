...
from app.utils.converters import to_decimal, to_int
...
def write_ohlcv(
    ...
    if existing:
        existing.close = to_decimal(data.get("close"))
        existing.high = to_decimal(data.get("high"))
        existing.low = to_decimal(data.get("low"))
